...
function _sm2Point(x, y, parity) {
    ...
    if (y != null) {
        ...
        let px = new BN(x, 16).toRed(SM2.red);
...
SM2KeyPair.prototype.signDigest = function(digest) {
    ...
    while (true) {
        let k = new BN(_drbg.generate(32, 'hex', utils.random(64)), 16).umod(this.curve.n);
        let kg = this.curve.g.mul(k);
        let r = utils.hashToBN(digest).add(kg.getX()).umod(this.curve.n);
        // r = 0
        if (r.isZero()) {
            continue;
        }
        ...
        let t1 = new BN(1).add(this.pri).invm(this.curve.n);
        let t2 = k.sub(r.mul(this.pri)).umod(this.curve.n);
        let s = t1.mul(t2).umod(this.curve.n);
        if (!s.isZero()) {
...
SM2KeyPair.prototype.verifyDigest = function(digest, r, s) {
    ...
    let t = bnr.add(bns).umod(this.curve.n);
    if (t.isZero()) {
        return false;
    }

    let q = this.curve.g.mul(bns).add(this.pub.mul(t));
    let R = utils.hashToBN(digest).add(q.getX()).umod(this.curve.n);
    if (!R.eq(bnr)) {
        return false;
    }
