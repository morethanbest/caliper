...
function privateToPublic(privateKey) {
    ...
    if(config.EncryptType == 1){
        ...
        let privateToPub = secp256k1.publicKeyCreate(privateKey, false).slice(1);
        return privateToPub;
    }
}
...
function ecsign(msgHash, privateKey) {
    let ret = {};
    if(config.EncryptType == 1){
        let _privateKey = privateKey.toString('hex');
        //console.log("SM2SignOrgData:",msgHash.toString("hex"));
        let _sign = gm.signRS(_privateKey,msgHash);
        ret.r = _sign.r;
        ret.s = _sign.s;
        ret.pub = _sign.pub;
    }else{
