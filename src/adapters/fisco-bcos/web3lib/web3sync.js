...
async function rawDeploy(account, privateKey, path, name, types, params) {
    ...
    let signTX = signTransaction(postdata, privateKey, null);
    return new Promise((resolve, reject) => {
...
function callByNameService(contract, func, version, params) {
    ...
    let strnamecallparams = JSON.stringify(namecallparams);
    ...
    let call_result = web3.eth.call(postdata);
...
async function sendRawTransaction(account, privateKey, to, func, params) {
    ...
    let tx_data = coder.codeTxData(func,types,params);
